---
title: GAS Home
description: Unreal GAS 主题下核心概念、Aura 实战和关联概念的导航页。
tags: [unreal, gas, wiki, moc]
updated: 2026-03-08
---

# GAS Home

`GAS` 已并入 `[[0 - Maps/Unreal Home|Unreal Home]]` 体系，这里只保留能力系统专题导航。

## 核心概念

- [[w/unreal/gas/asc|ASC]]
- [[w/unreal/gas/gameplayability|GameplayAbility]]
- [[w/unreal/gas/gameplayeffect|GameplayEffect]]
- [[w/unreal/gas/gameplayeffectspec|GameplayEffectSpec]]
- [[w/unreal/gas/gameplaytask|GameplayTask]]
- [[w/unreal/gas/abilitytask|AbilityTask]]
- [[w/unreal/gas/effectcontext|EffectContext]]
- [[w/unreal/gas/gas-prediction|GAS Prediction]]

## 动画、事件与目标

- [[w/unreal/gas/montage|Montage]]
- [[w/unreal/gas/montageevent|MontageEvent]]
- [[w/unreal/gas/虚幻事件声明|虚幻事件声明]]
- [[w/unreal/gas/fgameplayabilitytargetdata|FGameplayAbilityTargetData]]
- [[w/unreal/gas/gameabilitytargettypes|GameAbilityTargetTypes]]
- [[w/unreal/gas/predictionkey|PredictionKey]]

## 数值计算

- [[w/unreal/gas/mmc|MMC]]
- [[w/unreal/gas/execution-calculation|Execution Calculation]]
- [[w/unreal/gas/fgameplayeffectattributecapturedefinition|FGameplayEffectAttributeCaptureDefinition]]
- [[w/unreal/gas/fgameplaymodifierevaluateddata|FGameplayModifierEvaluatedData]]
- [[w/unreal/gas/metaattributes|MetaAttributes]]
- [[w/unreal/gas/sourceobject|SourceObject]]

## Aura 实战

- [[1 - Projects/Unreal/GAS/Aura/Aura Home|Aura Home]]
- [[w/unreal/gas/aura/伤害计算|伤害计算]]
- [[w/unreal/gas/aura/创建伤害类型|创建伤害类型]]
- [[w/unreal/gas/aura/使用effectcontext实现暴击效果传递|使用EffectContext实现暴击效果传递]]

## 关联概念

- [[w/unreal/gas/related/clientrpc|ClientRPC]]
- [[w/unreal/gas/related/netserialize|NetSerialize]]
- [[w/unreal/gas/related/unreal类型转换|Unreal类型转换]]
- [[w/unreal/gas/attributes/ufunction|UFUNCTION]]
- [[w/unreal/gas/attributes/uclass|UClass]]